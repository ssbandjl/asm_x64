;c06_core.asm:多处理器多任务内核，李忠，2022-04-23
...
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
search_for_a_ready_task:                          ;查找一个就绪的任务并将其置为忙
                                                  ;返回：R11=就绪任务的PCB线性地址
         ...
         swapgs
         mov r11, rbx
         cmp rbx, 0                               ;处理器当前未在执行任务？
         jne .again
         mov rbx, [rel pcb_ptr]                   ;是的。从链表首节点开始搜索。
         mov r11, rbx
  .again:
         mov r11, [r11 + 280]                     ;取得下一个节点
         xor rax, rax
         lock cmpxchg [r11 + 16], rcx             ;开发无锁算法的挑战并不是完全
         ;消除竞争，它可以归结为将代码的关键部分减少到由CPU本身提供的单个原子操作。
         jz .retrn
         cmp r11, rbx                             ;是否转一圈回到当前节点？
         je .fmiss                                ;是。未找到就绪任务（节点）
         jmp .again
...
resume_execute_a_task:                            ;恢复执行一个任务
         ...
         ;mov qword [r11 + 16], 1                  ;置任务状态为忙
         mov rbx, [r11 + 32]                      ;取PCB中的RSP0
...
time_slice_out_handler:                           ;时间片到期中断的处理过程
         ...
         lea rbx, [rbx + .return]                 ;将来恢复执行时，是从中断返回也～
...
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;新任务创建后，将广播新任务创建消息给所有处理器，所有处理器执行此中断服务例程。
new_task_notify_handler:                          ;任务认领中断的处理过程
         push rsi
         push r11
...
create_process:                                   ;创建新的任务
  ...
  .rdok:
         ...
         mov qword [r11 + 240], SUGG_PREEM_SLICE  ;推荐的任务执行时间片，来自global_defs.wid
...
init:    ;初始化内核的工作环境
  ...
  .ioapic:
   ...
   .m_end:
         ...
         mov dword [rsi + 0x320], 0x10000         ;定时器的本地向量表入口寄存器。
                                                  ;单次击发（one shot）模式
